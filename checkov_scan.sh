...
if ! command -v checkov &> /dev/null; then
    echo "ERROR: Checkov no está instalado."
    echo "Instalarlo con: pip install checkov"
    exit 1
fi

echo "OK - Checkov encontrado"
...
if [ $CHECKOV_EXIT_CODE -eq 0 ]; then
    echo "EXITO - Checkov validación correcta"
else
    echo "ADVERTENCIA - Checkov encontró problemas (código: $CHECKOV_EXIT_CODE)"
fi
...
# ===================================================
# Escaneos adicionales (comentados - descomenta si necesitas)
# ===================================================
